...
namespace SegurosFYP.Services
{
    public class DepartamentoServices
    {
        ...
        public DepartamentoServices(IOptions<Cliente> url, HttpClient client)
        {
            _url = url.Value;
            _client = client;
        ...
        public async Task<ServiceResult> Index()
        {
            ...
            try
            {
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    var list = JsonConvert.DeserializeObject<IEnumerable<DepartamentoViewModel>>(data); // Deserializar la cadena JSON en una lista de DepartamentoViewModel
