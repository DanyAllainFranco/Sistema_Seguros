    ...
    public class PersonaRepository : IRepository<tbPersonas>
    {
        ...
        public RequestStatus Insert(tbPersonas item)
        {
            ...
            using (var db = new SqlConnection(SegurosFYPContext.ConnectionString))
            {
                ...
                parameter.Add("@DniFrenteUrl", item.DniFrenteUrl);
                parameter.Add("@DniAtrasUrl", item.DniAtrasUrl);
                parameter.Add("@Poliz_Id", item.Poliz_Id);
                parameter.Add("@Paren_Id", item.Paren_Id);

                parameter.Add("@Perso_UsuarioCreacion", 1);
                parameter.Add("@Perso_FechaCreacion", DateTime.Now);
        ...
        public int Numeration(int top)
        {
            ...
            using (var db = new SqlConnection(SegurosFYPContext.ConnectionString))
            {
                list = db.Query<tbPersonas>(sql, new { Top = top }, commandType: CommandType.StoredProcedure).ToList();
